...
def getSublevel(pet):
    if isinstance(pet, PlayerPetInfo):
        ...
        return 0
PlayerPetInfo.__repr__ = PlayerPetInfo.__str__ = ShopPetInfo.__repr__ = ShopPetInfo.__str__ = lambda self: f"<{str(self.type)[8:]} L={getSublevel(self)} H={self.health} A={self.attack}>"
...
# N+1 calls to getOutcome
def permMetric(pets: List[PlayerPetInfo], currBest: int):
    N = 15
    total = 100 * getOutcome(pets, G.next_opponent_info.pets)
    if total + N * 1 <= currBest: return total
    ...
    for i in range(N):
        ...
        for _ in range(3):
            opp[randint(0, 4)] = OtherPlayerPetInfo({
                "id": 696969,
        ...
        if total + (N - 1 - i) * 1 <= currBest: return total
    return total
...
def STAGE_perm():
    ...
    best = -696969
    hasElephant = len(collectPets(PetType.ELEPHANT)) > 0
    hasCamel = len(collectPets(PetType.CAMEL)) > 0
    seen = set()
    for perm in permutations(range(5)):
        pets = [G.player_info.pets[i] for i in perm]
        hash_pets = str([pet for pet in pets if pet])
        if hash_pets in seen: continue
        seen.add(hash_pets)
        if hasElephant and hasCamel:
            if not pets[0] or pets[0].type != PetType.ELEPHANT: continue
            if not pets[1] or pets[1].type != PetType.CAMEL: continue
        elif hasCamel:
            if not pets[0] or pets[0].type != PetType.CAMEL: continue
        elif hasElephant:
            if not pets[4] or pets[4].type != PetType.ELEPHANT: continue
        
        outcome = permMetric(pets, best)
        if outcome > best:
            best = outcome
            bestPerm = perm
...
def level_pet(pet_a, pet_b):
    if pet_a in G.player_info.shop_pets:
        bot_battle.level_pet_from_shop(pet_a, pet_b)
    elif pet_a in G.player_info.pets:
        bot_battle.level_pet_from_pets(pet_a, pet_b)
    else:
        raise Exception

def STAGE_buy_merge_all(wanted_type: PetType, wanted_pet: Optional[ShopPetInfo]):
    pets = collectPets(wanted_type)
    if wanted_pet:
        pets.append(wanted_pet)
    for to_merge in powerset(pets):
        if len(to_merge) >= 2 and calc_sublevel(to_merge) in [2, 5]:
            for pet in to_merge[1:]:
                level_pet(getPet(pet), getPet(to_merge[0]))
                globals()["G"] = bot_battle.get_game_info()
            return True
    return False
...
def STAGE_buy_merge_any():
    ...
    #         return True
    for pet in G.player_info.pets:
        if not pet or pet.type == PetType.BUNNY: continue
        if STAGE_buy_merge_all(pet.type, None):
            return True
    return False
...
def STAGE_buy_exchange(wanted_pet: ShopPetInfo):
    ...
    if best[0] < petMetric(wanted_pet)[0]:
...
def STAGE_buy_pet():
    ...
    for wanted_pet in wanted_pets:
        sublevel = calc_sublevel(collectPets(wanted_pet.type))
        if sublevel >= 5: continue
        if wanted_pet.type == PetType.BUNNY and sublevel >= 2: continue
        if petMetric(wanted_pet)[0] <= 0: continue
        
        if STAGE_buy_insert(wanted_pet): return True
        if STAGE_buy_exchange(wanted_pet): return True
        if STAGE_buy_merge(wanted_pet.type, wanted_pet): return True
        if STAGE_buy_merge_any() and STAGE_buy_insert(wanted_pet): return True
    return False
...
def STAGE_freeze_food():
    for food in G.player_info.shop_foods:
        if foodMetric(food) >= 2 and not food.is_frozen:
...
def STAGE_buy_food():
    ...
    if collectPets(PetType.BUNNY) and collectPets(PetType.PEACOCK):
        for food in sorted(G.player_info.shop_foods, key=foodMetric, reverse=True):
            if foodMetric(food) > 0:
                peacock = collectPets(PetType.PEACOCK)[0]
                bot_battle.buy_food(getFood(food), getPet(peacock))
                globals()["G"] = bot_battle.get_game_info()
                return True
            
    # still havent seen bunny, but food is essential for endgame
    elif G.round_num >= 8:
        for food in sorted(G.player_info.shop_foods, key=foodMetric, reverse=True):
            if foodMetric(food) < 2: continue
            for target in sorted(G.player_info.pets, key=AHMetric, reverse=True):
                if target and not target.carried_food:
                    bot_battle.buy_food(getFood(food), getPet(target))
                    globals()["G"] = bot_battle.get_game_info()
                    return True
    return False
...
def STAGE_unfreeze():
    for pet in G.player_info.shop_pets:
        if pet.is_frozen:
            ...
            globals()["G"] = bot_battle.get_game_info()
    for food in G.player_info.shop_foods:
        if foodMetric(food) < 2 and food.is_frozen:
...
def petMetric(shop_pet: ShopPetInfo):
    if G.round_num <= 2:
        ...
        rating = {
            ...
            # tier 3
            PetType.BUNNY: +2,
            PetType.CAMEL: +1, # hurt
            # tier 4
            PetType.ELEPHANT: +1
        }
        want = rating.get(shop_pet.type, -1)
    return (want, shop_pet.health * shop_pet.attack)

def foodMetric(food: ShopFoodInfo):
    rating = {
        # tier 2
        FoodType.CUPCAKE: +1,
        FoodType.MEAT_BONE: +2,
        # tier 3
        FoodType.GARLIC: +2,
        # tier 4
        FoodType.PEAR: +2 # should this be 2? i dont see other players using it
    }
    return rating.get(food.type, -1)
...
def main():
    for round_num in count(1):
        ...
        while True:
            if G.round_num >= 8:
                while STAGE_buy_merge_any(): continue
