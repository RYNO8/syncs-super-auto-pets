...
def getSublevel(*pets):
    ...
    for pet in pets:
        if isinstance(pet, PlayerPetInfo):
            total += 1 + pet.sub_level + LEVEL_2_CUTOFF * (pet.level == 2) + LEVEL_3_CUTOFF * (pet.level == 3)
        elif isinstance(pet, ShopPetInfo):
            total += 1
    return total
...
def petMetric(pet: Optional[ShopPetInfo]) -> float:
    ...
    if sublevel == 6 and collectPets(PetType.BISON):
        adjust += haMetric(2 * remaining_rounds, remaining_rounds)
...
def permMetric(pets: List[PlayerPetInfo], H=3, A=3):
    ...
    score = 0
    # tiebreak so that high stat pets are at the front
    score += 0.01 * sum(petMetric(pet) * (5 - i) for i, pet in enumerate(pets))
...
class Action:
    ...
    def exec(self, threshold = 1):
        if self.score >= threshold and G.player_info.coins >= self.cost:
            ...
            return self.exec_fail()
        else:
            return False
    
    def exec_fail(self):
        if not self.fail:
            return False # this is a nothing
        else:
            print("FREEZE", self)
            for f in self.fail:
                f()
    ...
    def nothing():
        return Action("NOTHING", -100000000, [], 0, [])
...
def level_pet(pet_to_use, pet_to_level_up, order=True) -> Action:
    pet_to_use = getPet(pet_to_use)
    pet_to_level_up = getPet(pet_to_level_up)
    if getSublevel(pet_to_use) == 6 or getSublevel(pet_to_level_up) == 6:
        return Action.nothing()
    ...
    if pet_to_use in G.player_info.shop_pets:
        ...
        elif petMetric(pet_to_use) < petMetric(pet_to_level_up):
            return Action.level_pet_from_pets(pet_to_use, pet_to_level_up)
        else:
            return Action.level_pet_from_pets(pet_to_level_up, pet_to_use)
...
def fish_merge() -> Action:
    fishes = sorted(collectPets(PetType.FISH), key=petMetric)
    # fishes = [fish for fish in fishes if getSublevel(fish) < 6]
    # # for a, b in [(1, 2), (3, 3), (1, 1), (2, 2), (2, 4)]:
    # #     for fish_a in fishes:
    # #         for fish_b in fishes:
    # #             if fish_a != fish_b and getSublevel(fish_a) == a and getSublevel(fish_b) == b:
    # #                 return level_pet(fish_a, fish_b)
    # if len(fishes) >= 2:
    #     return level_pet(fishes[0], fishes[-1])
    # return Action.nothing()
    if len(fishes) >= 5: level_pet(fishes[4], fishes[0], order=False).exec(-6969)
    if len(fishes) >= 4: level_pet(fishes[3], fishes[1], order=False).exec(-6969)
    if len(fishes) >= 3: level_pet(fishes[2], fishes[0], order=False).exec(-6969)
    if len(fishes) >= 2: level_pet(fishes[1], fishes[0], order=False).exec(-6969)
...
def find_best_merge() -> Action:
    pets = [pet for pet in G.player_info.pets if pet and pet.type != PetType.FISH]
    
    action = Action.nothing()
    for pet in pets:
        if not pet or pet.type == PetType.FISH: continue
        action |= buy_merge(*collectPets(pet.type))
    return action

def find_best_action() -> Action:
    pets = [pet for pet in G.player_info.pets if pet and pet.type != PetType.FISH]
    shop_pets = [shop_pet for shop_pet in G.player_info.shop_pets if shop_pet and not shop_pet.is_frozen and shop_pet.type != PetType.FISH]
...
def opening_strat() -> Action:
    fishes = [shop_pet for shop_pet in G.player_info.shop_pets if shop_pet and not shop_pet.is_frozen and shop_pet.type == PetType.FISH]
    if not fishes:
        return Action.nothing()
    fish = fishes[0]
    for i in range(5):
        if not G.player_info.pets[i]:
            return Action.buy_pet(fish, i)
    for i in range(5):
        if getSublevel(G.player_info.pets[i]) == 1:
            return level_pet(fish, G.player_info.pets[i])
    return Action.nothing()
...
def main():
    for round_num in count(1):
        ...
        while True:
            if G.round_num < 5:
                while True:
                    action = opening_strat()
                    if G.round_num + 1 < 5 and G.player_info.coins <= 3:
                        # freeze pet so we have more rolls
                        if not action.exec_fail(): break
                    else:
                        if not action.exec(): break
            else:
