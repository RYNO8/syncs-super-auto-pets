...
def permMetric(perm: List[int], currBest: int):
    ...
    if total + 30 * 1 <= currBest: return total
    ...
    for i in range(30):
        ...
        if total + (29 - i) * 1 <= currBest: return total
...
def STAGE_perm():
    ...
    for perm in permutations(range(5)):
        if G.player_info.pets[perm[0]].type == PetType.KANGAROO:
            continue
...
def petMetric(shop_pet: ShopPetInfo):
    if calc_sublevel(collectPets(shop_pet.type)) >= 5:
        want = -2
    elif G.round_num <= 2:
        rating = {
            # tier 1
...
def foodMetric(food: ShopFoodInfo):
    if food.type in [FoodType.GARLIC, FoodType.MEAT_BONE, FoodType.GARLIC, FoodType.PEAR, FoodType.CANNED_FOOD]:
        return +1
    else:
        return -1
...
def main():
    for round_num in count(1):
        ...
        while G.player_info.coins >= 3:
            ...
            while STAGE_buy_food(): continue
            if G.round_num == 1 and sum(pet is not None for pet in G.player_info.pets) < 3 and G.player_info.coins == 3:
