...
game_info: GameInfo = bot_battle.get_game_info()
...
def permMetric(perm: List[int]):
    pets = [game_info.player_info.pets[i] for i in perm]
    total = getOutcome(pets, game_info.next_opponent_info.pets)
    # for player in game_info.other_players_info:
    #     total += getOutcome(pets, player.pets)
    return total
...
def get_perm(perm):
    ...
    for action in actions:
        ...
        globals()["game_info"] = bot_battle.get_game_info()
...
def STAGE_perm():
    ...
    # print([game_info.player_info.pets[i].type for i in bestPerm if game_info.player_info.pets[i]])
    # print(best)
    return get_perm(bestPerm)

def STAGE_buy_insert(wanted_pet: ShopPetInfo):
    if None in game_info.player_info.pets:
        i = game_info.player_info.pets.index(None)
...
def calc_sublevel(pets: List[PlayerPetInfo]):
    return sum(pet.sub_level for pet in pets) + len(pets) - 1
...
def petMetric(pet: PlayerPetInfo):
    if pet.type == PetType.BEAVER:
        return -1
    else:
        return 0
...
def shopPetMetric(shop_pet: ShopPetInfo):
    AH = shop_pet.health * shop_pet.attack
    if shop_pet.type in [PetType.BADGER, PetType.HEDGEHOG, PetType.ELEPHANT]:
        wantedness = -2
    elif any(pet.type == shop_pet.type and pet.level == 3 for pet in game_info.player_info.pets if pet):
        wantedness = -2
    elif round_num <= 6:
        if shop_pet.type in [PetType.PEACOCK, PetType.FISH] or AH >= 10:
            wantedness = +1
        elif shop_pet.type in [PetType.BEAVER]:
            wantedness = 0
        else:
            wantedness = -1
    else:
        if shop_pet.type in [] or AH >= 10:
            wantedness = +1
        elif shop_pet.type in []:
            wantedness = 0
        else:
            wantedness = -1
    return (wantedness, AH)

def STAGE_buy():
    global game_info
    if game_info.player_info.coins < 3: return False

    shop_pets = game_info.player_info.shop_pets
    assert all(shop_pet.cost == 3 and not shop_pet.is_frozen for shop_pet in shop_pets)
    wanted_pets = sorted(shop_pets, key=shopPetMetric, reverse=True)
    for wanted_pet in wanted_pets:
        wantedness = shopPetMetric(wanted_pet)[0]
        if wantedness in [0, +1] and STAGE_buy_insert(wanted_pet): return True
        if wantedness in [0, +1] and STAGE_buy_merge_any() and STAGE_buy_insert(wanted_pet): return True
        if wantedness in [+1] and STAGE_buy_merge(wanted_pet.type, wanted_pet): return True
        if wantedness in [+1] and STAGE_buy_exchange(wanted_pet): return True
    return False
...
def STAGE_fill():
    ...
    for wanted_pet in wanted_pets:
        wantedness = shopPetMetric(wanted_pet)[0]
        if wantedness in [-1, 0, +1] and STAGE_buy_insert(wanted_pet): return True
    return False

def foodMetric(food: ShopFoodInfo):
    if food.type in [FoodType.MEAT_BONE, FoodType.GARLIC, FoodType.PEAR]:
        return +1
    else:
        return 0
...
def STAGE_freeze_food():
    ...
    for food in game_info.player_info.shop_foods:
    ...
    for food in game_info.player_info.shop_foods:
        for pet in game_info.player_info.pets:
            if food.is_frozen and pet and not pet.carried_food:
                bot_battle.buy_food(food, getPet(pet))
                globals()["game_info"] = bot_battle.get_game_info()
                return True
    return False
...
def STAGE_feeze():
    global game_info
    if game_info.player_info.coins == 0: return False
...
for round_num in count(1):
    print("#" * 20, round_num, "#" * 20)
    print()
    while True:
        while STAGE_buy(): continue
        while STAGE_freeze_food(): continue
        if game_info.player_info.coins >= 4:
            bot_battle.reroll_shop()
            globals()["game_info"] = bot_battle.get_game_info()
        else:
            break

    # while STAGE_feeze(): continue
    STAGE_give_food()
    STAGE_fill()
    STAGE_perm()
